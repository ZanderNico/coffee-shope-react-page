...
function Recipes() {
  return (
    <div className="flex flex-col bg-gray-900 justify-center items-center pt-5 pb-40">
      <h1 className="text-5xl font-bold mt-20 mb-5 text-white">Coffee Recipes</h1>
      <div className="w-full md:w-[600px] lg:w-[900px]">
        <p className="italic font-mono text-center text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10 mt-20">
        {sampleCoffeeData.map((coffee, index) => (
          <div
            key={index}
            className="overflow-hidden p-5 shadow-md bg-white border"
          >
            <img
              src={coffee.image}
              alt={coffee.title}
            ...
            <div className="p-4">
              ...
              <p className="text-gray-700 text-center underline">read more</p>
            </div>
          </div>
        ))}
      </div>
